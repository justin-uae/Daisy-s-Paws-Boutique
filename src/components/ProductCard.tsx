import { ShoppingBag, Package, PawPrint } from 'lucide-react';
...
const ProductCard = ({ product, goToDetail, formatPrice }: any) => {
    ...
    return (
        ...
        >
            ...
            <div
                className={`relative overflow-hidden flex items-center justify-center ${isOutOfStock
            ...
            >
                <img
                    ...
                    alt={product.title}
                    className={`w-full h-auto max-h-[400px] object-contain transition-transform duration-500 ${isOutOfStock
                ...
                {/* Price Badge */}
                <div
                    className={`absolute bottom-3 left-3 rounded-xl px-3 py-2 shadow-xl z-10 ${isOutOfStock
                ...
                >
                    <div className="flex flex-col">
                        <span
                            className={`text-xs line-through leading-tight ${isOutOfStock ? 'text-gray-500' : 'text-[#8B5A3C]'
                                }`}
                        >
                            {formatPrice(product.price + 10)}
                        </span>
                        <span
                            className={`text-lg font-black leading-tight ${isOutOfStock
                                    ? 'text-gray-600'
            ...
            <div className="p-4">
                {product.category && (
                    <div className="flex items-center gap-1.5 mb-2">
                        <PawPrint
                            className={`w-3.5 h-3.5 flex-shrink-0 ${isOutOfStock ? 'text-gray-400' : 'text-[#8B5A3C]'
                                }`}
                        />
                        <span
                            className={`text-xs sm:text-sm font-semibold truncate ${isOutOfStock ? 'text-gray-500' : 'text-[#6D4C41]'
                                }`}
                        >
                            {product.category}
                        </span>
                ...
                <h3
                    className={`text-base md:text-lg font-bold transition-colors line-clamp-2 leading-tight mb-2 min-h-[2.5rem] ${isOutOfStock
                            ? 'text-gray-600'
                ...
                <p
                    className={`text-sm line-clamp-2 mb-3 ${isOutOfStock ? 'text-gray-500' : 'text-[#6D4C41]'
                        }`}
                >
                    {product.description}
                </p>
